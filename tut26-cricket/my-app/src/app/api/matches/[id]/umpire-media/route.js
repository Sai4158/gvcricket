...
export async function GET(_req, { params }) {
  try {
    ...
    const match = await Match.findById(id).select(
      "_id matchImages matchImageUrl matchImagePublicId matchImageStorageUrlEnc matchImageStorageUrlHash matchImageUploadedAt matchImageUploadedBy mediaUpdatedAt updatedAt adminAccessVersion lastLiveEvent lastEventType lastEventText",
    );
    if (!match) {
    ...
    return jsonError("Could not load umpire media.", 500);
  }
}
